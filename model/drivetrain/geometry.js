...
const cfg={length:120,width:54,track:43,wheelbase:66,deck:24,arm:16,kingpinZ:13.5,kpi:12*Math.PI/180,kingpinD:1.75,pivotSpan:10,legIn:4.5,steerArm:7,ackAng:12*Math.PI/180,armUp:3,rackHalf:5,pivotX:49,receiverGap:28,railInset:14,sledX:-5,
 ...
 packL:50,packW:21,packH:10.5,packBottom:8.5,bagH:6.5,bagMin:2.8,bagMax:10.5,bagD:7,motionRatio:10/16,unsprungLb:110,bagMaxLb:2096};
...
const parts={
 frame:{title:'Welded ladder frame',kind:'Fabricate',text:'2 × 4 in side rails at ±12 in (moved 2 in inboard after the review so the tire chord at rail height clears the rail at full lock); crossmembers and bolted corner mounting plates. Sleeve and motor loads need a complete frame calculation.',source:'../drivetrain-review.md'},
 receivers:{title:'Receiver sleeves',kind:'Fabricate',text:'2½ × 3½ in sleeves for 2 × 3 in pod arms on edge, 28 in center spacing (was 26: the axle walks ~1 in toward the pivot at full kneel and the tire chord at sleeve height grows). Pods omitted.'},
 arms:{title:'Suspension arms & hangers',kind:'Fabricate + machine',text:'16 in pivot-to-kingpin arms on 10 in wide coaxial pivots. Front A-arm legs both sit inboard of the kingpin plane (4.5 and 10 in) so the tire can sweep to lock; a tip cross tube reaches out to the kingpin. Front trailing / rear leading arms are mirrored; this motion changes front caster.'},
 ...
 knuckle:{title:'Steering knuckle & torque block',kind:'Machine',text:'Kingpin bearing seats, thrust faces and clamped antirotation interface to the stationary motor shaft. This is the main custom precision assembly. Kingpin 8 in inboard of the hub centre (about the least this single-sided package allows), inclined 12° so the axis meets the ground ~5.6 in inboard of the contact centre; 1¾ in pin. Bending at the corner load is in the readout.'},
 ...
 steering:{title:'Rack, tie rods & column',kind:'Buy + custom geometry',text:'Steering arms point forward and outward; the rack sits on the frame at the ball joints\' own height and station, so the transverse rods are square to the ball joints\' arc and toe change over travel is second order. Fixed-length tie rods solve independent wheel angles; no catalog rack selected yet. Column uses supported shafts and U-joints.'},
 ...
 battery:{title:'Two 16S battery strings',kind:'Buy cells + build enclosure',text:'32 LF280K cells shown at 72×173.7×207.5 mm; 28.672 kWh nominal, ~23–26 kWh in an 80–90 % window. Reserved enclosure 50×21×10½ in, shifted 5 in rearward. Separate pack protection and precharge, insulation and mechanical retention require detailed design.',source:'https://www.evebatteryusa.com/_files/ugd/abe55e_7ba2e98dd2884f8d978b861be2b471e2.pdf'},
...
const X_=new T.Vector3(1,0,0),Y_=new T.Vector3(0,1,0),Z_=new T.Vector3(0,0,1);
// knuckle-fixed point given in level orientation (at straight-ahead) -> world: steer about the inclined kingpin, then swing with the arm
function knucklePoint(off,s,theta,k){const a=-Math.asin(k/cfg.arm);return V(off).applyAxisAngle(X_,s*cfg.kpi).applyAxisAngle(Y_,theta).applyAxisAngle(X_,-s*cfg.kpi).add(V([-16,0,0])).applyAxisAngle(Z_,a).add(V([cfg.pivotX,cfg.tireD/2-k,s*cfg.kingpinZ]));}
const armEndOff=s=>[cfg.steerArm*Math.cos(cfg.ackAng),cfg.armUp,s*cfg.steerArm*Math.sin(cfg.ackAng)];
function steeringPoint(s,theta,k){return knucklePoint(armEndOff(s),s,theta,k);}
const rackX=cfg.pivotX-cfg.arm+cfg.steerArm*Math.cos(cfg.ackAng),rackY=cfg.tireD/2+cfg.armUp;
const rackPoint=(s,travel,k)=>V([rackX,rackY-k,s*cfg.rackHalf+travel]);
...
function solveAngle(s,travel,k){const f=t=>steeringPoint(s,t,k).distanceTo(rackPoint(s,travel,k))-rodLength(s);let roots=[],best={theta:0,error:Infinity};let lo=-55*rad,fl=f(lo);
 ...
 const expected=travel/cfg.steerArm;roots.sort((a,b)=>Math.abs(a-expected)-Math.abs(b-expected));return roots.length?{theta:roots[0],error:Math.abs(f(roots[0])),valid:true}:{...best,valid:false};}
function state(input={}){const k=input.kneel||0,travel=input.rack||0;const solutions=[-1,1].map(s=>({s,...solveAngle(s,travel,k)}));const R=cfg.tireD/2;
 const rpm=5*.44704/(Math.PI*cfg.tireD*.0254)*60,m=input.mass||2000,g=9.80665;
 const tRoll=m*g*(input.crr||.06)*(R*.0254)/4,tGrade=m*g*((input.grade||0)/100)*(R*.0254)/4,tAccel=m*(input.accel||0)*(R*.0254)/4,torque=tRoll+tGrade+tAccel;
 // range at 5 mph: rolling + grade + a headwind on ~8 m² of frontal area (aero is negligible in still air at 5 mph, not in a 20 mph wind), divided by battery-to-wheel efficiency
 const v=5*.44704,vRel=v+(input.wind||0)*.44704,fAero=.5*1.2*vRel*vRel*(input.cda||8),fRoad=m*g*((input.crr||.06)+(input.grade||0)/100)+fAero;
 const eta=input.eta||.7,whPerMile=fRoad*1609.34/3600/eta,kwMoving=fRoad*v/1000/eta;
 const usable=input.usable||24.5,other=input.other||6,driveKwh=Math.max(0,usable-other),miles=driveKwh*1000/whPerMile,hours=miles/5;
 const range={whPerMile,kwMoving,driveKwh,miles,hours,fAero,byCrr:[.03,.06,.1].map(c=>{const f=m*g*(c+(input.grade||0)/100)+fAero;return{crr:c,miles:driveKwh*1000/(f*1609.34/3600/eta)}})};
 const scrubHub=cfg.track/2-cfg.kingpinZ,scrubGround=scrubHub-R*Math.tan(cfg.kpi);
 return{k,travel,explode:input.explode||0,solutions,rpm,torque,tRoll,tGrade,tAccel,scrubHub,scrubGround,range,bagH:cfg.bagH-k*cfg.motionRatio,clearance:cfg.packBottom-k,rodLengths:[rodLength(-1),rodLength(1)]};}
function build(input={}){
 ...
 const railZ=cfg.width/2-cfg.railInset-1;for(const s of [-1,1]){const m=box(chassis,[0,22,s*railZ],[118,4,2],'steel','frame','side_rail');collisionBoxes.push(m)}
 for(const x of [-59,-48.25,-19,0,19,48.25,59])box(chassis,[x,22.5,0],[2,3,2*railZ-2],'steel','frame','crossmember');
 for(const e of [-1,1])for(const x of [cfg.pivotX-16-cfg.receiverGap/2,cfg.pivotX-16+cfg.receiverGap/2])for(const s of [-1,1]){const m=box(chassis,[e*x,22,s*19],[2.5,3.5,16],'fab','receivers','receiver_sleeve');collisionBoxes.push(m);box(chassis,[e*x,22,s*27.02],[1.98,2.98,.06],'black','receivers','receiver_opening');bolt(chassis,[e*x,22,s*25],'x','receivers',3,.24)}
 for(const e of [-1,1])for(const s of [-1,1])box(chassis,[e*52,22,s*cfg.receiverGap/2],[16,3.5,2.5],'fab','receivers','end_receiver');
 ...
 const battery=new T.Group();battery.name='BATTERY_SLED';battery.position.x=cfg.sledX;chassis.add(battery);
 ...
 for(const x of [cfg.sledX-22,cfg.sledX+22])for(const s of [-1,1]){box(chassis,[x,19.7,s*11.5],[4,.5,4],'fab','battery','sled_bolt_tab');bolt(chassis,[x,19.9,s*11.5],'y','battery',1.5,.25)}
 anchors.battery=V([cfg.sledX,19,0]);anchors.frame=V([-53,24,-railZ]);anchors.receivers=V([-19,23,-25]);
 const cornerGroups=[];
 for(const e of [-1,1])for(const s of [-1,1]){
  ...
  const R=cfg.tireD/2,az=e===1?cfg.kingpinZ:14,px=e*cfg.pivotX,pivotY=R-q.k,ps=cfg.pivotSpan,kpi=e===1?cfg.kpi:0;
  ...
  for(const z of [s*(az-ps),s*az]){box(cg,[px,(pivotY+19-q.k)/2,z],[4,19-q.k-pivotY,.5],'fab','arms','pivot_hanger');cyl(cg,[px,pivotY,z],1.05,1.2,'z','buy','pivots','bushing');washer(cg,[px,pivotY,z+s*.7],1.13,.52,.12,'z','bright','pivots','thrust_washer')}
  cyl(cg,[px,pivotY,s*(az-ps/2)],.5,ps+2,'z','machine','pivots','pivot_shaft');
  ...
  // Front: both legs inboard of the kingpin plane so the tire can sweep to lock; a tip cross tube reaches the kingpin. Rear: L-arm beside the motor.
  const legs=e===1?[-s*cfg.legIn,-s*ps]:[0,-s*ps];
  for(const dz of legs){const m=box(arm,[-e*8,0,dz],[16,2.5,1.5],'fab','arms','arm_leg');collisionBoxes.push(m)}
  box(arm,[-e*15.25,0,-s*(ps/2-(e===1?.375:.25))],[1.5,2.5,e===1?ps+1.5:ps-.5],'fab','arms','arm_cross_tube');
  cyl(arm,[0,0,-s*ps/2],.88,ps-.5,'z','fab','pivots','pivot_sleeve');
  // Knuckle: kt is the inclined kingpin frame on the arm tip; kp steers about it; wl is the level frame the wheel end is built in.
  const kt=new T.Group();kt.name='KINGPIN_AXIS';kt.position.set(-e*16,0,0);kt.rotation.x=-s*kpi;arm.add(kt);
  const sol=e===1?q.solutions.find(a=>a.s===s):{theta:0};
  const kp=new T.Group();kp.name='STEER_KNUCKLE';kp.rotation.y=sol.theta;kt.add(kp);
  const wl=new T.Group();wl.name='WHEEL_END_LEVEL';wl.rotation.x=s*kpi;kp.add(wl);
  if(e===1){for(const yy of [-3.75,3.75]){box(kt,[0,yy,0],[3.5,.5,3.5],'fab','knuckle','kingpin_ear');cyl(kp,[0,yy*.75,0],cfg.kingpinD/2+.35,1.2,'y','buy','knuckle','kingpin_bushing');washer(kp,[0,yy*.85,0],cfg.kingpinD/2+.55,cfg.kingpinD/2+.01,.2,'y','bright','knuckle','kingpin_thrust')}
   cyl(kt,[0,0,0],cfg.kingpinD/2,9,'y','machine','knuckle','kingpin');box(kp,[0,0,0],[3.2,5,3.2],'machine','knuckle','knuckle_body');
   tube(wl,[0,cfg.armUp,0],armEndOff(s),1,1,'machine','steering','steering_arm');
  }
  ...
  box(wl,[0,0,s*(1.6+wheelOffset-3.1)/2],[3.1,3.4,wheelOffset-3.1-1.6],'machine','knuckle','shaft_torque_block');for(const xx of [-1,1])for(const yy of [-1.15,1.15])bolt(wl,[xx,yy,s*(wheelOffset-3)],'z','knuckle',1,.18);
  cyl(wl,[0,0,s*(wheelOffset-2.3)],mm(28)/2,4.6,'z','bright','motor','stationary_shaft');
  cyl(wl,[0,0,s*(wheelOffset-4.1)],mm(24)/2,mm(25),'z','bright','motor','M24_thread_envelope');
  const wm=new T.Group();wm.name='ROTATING_MOTOR';wm.position.z=s*(wheelOffset+spread*5);wl.add(wm);
  ...
  const rotorZ=s*(wheelOffset-2.6+spread*2);cyl(wm,[0,0,-s*1.9],1.6,1.2,'z','bright','brakes','rotor_hat_allowance');washer(wl,[0,0,rotorZ],3.55,1.18,.25,'z','bright','brakes','180mm_rotor_allowance');
  for(let n=0;n<18;n++){const a=n*Math.PI/9;cyl(wl,[2.9*Math.cos(a),2.9*Math.sin(a),rotorZ+s*.135],.12,.025,'z','black','brakes','rotor_drill_mark',8)}
  box(wl,[2.8,1.8,rotorZ],[2.2,2.3,1.6],'buy','brakes','service_caliper_allowance');box(wl,[1.7,.9,rotorZ-s*1],[3,.35,1.6],'machine','brakes','caliper_adapter');
  if(e===-1){box(wl,[-2.8,1.3,rotorZ],[2,2,1.65],'buy','park','mechanical_parking_caliper');bar(wl,[-3.2,1.8,rotorZ],[-4.7,3.5,rotorZ],.16,'bright','park','parking_lever')}
  const rim=new T.Group();rim.name='12_IN_RIM';rim.position.z=s*(wheelOffset+spread*11);wl.add(rim);
  ...
  const tire=new T.Group();tire.name='CARLSTAR_574353';tire.position.z=s*(wheelOffset+spread*18);wl.add(tire);
  ...
  wheels.push({tire,kp:wl,e,s});
  // Spring base moves with the rigid arm, between the legs. Top is tied to the sprung module.
  const zs=e===1?-s*(cfg.legIn+ps)/2:-s*3.5;
  const low=V([-e*10,2.25,zs]).applyMatrix4(new T.Matrix4().compose(arm.position,arm.quaternion,arm.scale));const high=V([px-e*10,R+2.25+cfg.bagH-q.k,s*az+zs]);
  box(cg,high.toArray(),[8,.5,8],'fab','spring','upper_spring_plate');
  for(const z of [s*az+zs-s*3.9,s*az+zs+s*3.9])tube(cg,[high.x,high.y,z],[px,20-q.k,z],.6,.8,'fab','spring','tower_brace');
  const bag=new T.Group();bag.name='D2500';bag.userData={part:'spring'};cg.add(bag);const d=high.clone().sub(low),h=d.length();bag.position.copy(low);bag.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),d.normalize());
  ...
  collisionBoxes.push(bag);
  const zd=e===1?-s*(ps+2):-s*7;
  const db=V([-e*5,0,zd]).applyMatrix4(new T.Matrix4().compose(arm.position,arm.quaternion,arm.scale)),dt=V([px-e*1,19-q.k,s*az+zd]),mid=db.clone().lerp(dt,.6);
  ...
  cyl(cg,[px-e*12,17-q.k,s*(az-(e===1?cfg.legIn:0))],.8,2,'y','black','damper','bump_stop');path(cg,[[px-e*7,19-q.k,s*(az-6)],[px-e*10,14-q.k/2,s*(az-6)],[-e*7+px,pivotY+1,s*(az-6)]],.08,'black','damper','droop_strap');
  ...
  const motorWireStart=V([0,0,s*(wheelOffset-4)]);wl.updateWorldMatrix(true,false);motorWireStart.applyMatrix4(wl.matrixWorld);
  ...
  if(e===1&&s===1){anchors.motor=V([33,R,23]);anchors.knuckle=V([33,R+2,cfg.kingpinZ]);anchors.spring=high.clone();anchors.wheel=V([33,R,29]);anchors.brakes=V([36,R+2,19]);}
 }
 // Rack is frame mounted at the ball joints' station and height (square to their arc). Inner pivot spacing is a proposal.
 box(chassis,[rackX,rackY,0],[2.1,2.1,12],'buy','steering','rack_housing');
 for(const s of [-1,1]){cyl(chassis,[rackX,rackY,s*cfg.rackHalf+q.travel],.38,4,'z','bright','steering','rack_bar');for(let n=0;n<7;n++)ring(chassis,[rackX,rackY,s*(cfg.rackHalf-1+n*.2)+q.travel/2],.68,.11,'z','black','steering','rack_boot');tube(chassis,[rackX,rackY,s*3],[rackX,21,s*(railZ-1)],.6,.6,'fab','steering','rack_mount')}
 const col=[[rackX,rackY,2],[50,17,6],[57,25,8],[68,38,8],[72,43,8]];
 ...
 // Sample tire volume vs rail, receiver, arm-leg and air-spring boxes. A limited interference screen, not certification.
